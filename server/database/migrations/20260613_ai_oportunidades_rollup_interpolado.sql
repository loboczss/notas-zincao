-- Enquanto a tabela diaria ainda nao foi reconstruida pelo sync novo,
-- estima periodos como 7, 15 e 60 dias a partir dos rollups acumulados
-- existentes (30/90/180/365). Quando houver venda diaria real, ela prevalece.
...
begin
  ...
  if position(old_metrics in ddl) = 0 then
    raise exception 'expected fixed-rollup metrics snippet not found in integrim_produto_valor_periodo';
  end if;
  if position(old_join in ddl) = 0 then
    raise exception 'expected daily_state join snippet not found in integrim_produto_valor_periodo';
  end if;

...
import type { NotaImageChaveResponse } from '../../../../shared/types/NotasRetirada'
import { extractNotaLookupHintsFromImage } from '../../../services/openai'
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    return {
      success: true,
      hints,
    } satisfies NotaImageChaveResponse
  }
  catch (error) {
    console.error('[api/notas/integrim/chave-from-image] error:', error instanceof Error ? error.message : 'unknown error')
    throw createError({
      statusCode: 502,
      statusMessage: 'Nao foi possivel ler a chave da foto.',
    })
  }
})

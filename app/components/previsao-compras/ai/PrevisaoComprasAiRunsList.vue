...
<template>
  <Card padding-class="p-0" class="overflow-hidden">
    ...
    <template v-else>
      <!-- Lista Mobile (lg:hidden) -->
      <div class="lg:hidden divide-y divide-slate-100 dark:divide-slate-800">
        <div
          v-for="run in props.runs"
          :key="run.id"
          class="p-4 hover:bg-slate-50/30 dark:hover:bg-slate-900/20 transition-colors space-y-3"
        >
          ...
          <div class="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 border-t border-slate-50 dark:border-slate-800/40 pt-2">
            <div>
              Fontes: <span class="font-bold text-slate-800 dark:text-slate-200">{{ run.sources_count }}</span>
            </div>
            <div>
              Oportunidades: <span class="font-bold text-slate-800 dark:text-slate-200">{{ run.opportunities_count }}</span>
            </div>
          </div>
        </div>
      </div>

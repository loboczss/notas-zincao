...
const statusInfo = computed(() => {
  ...
  if (dias !== null && dias < 45) {
    return {
      label: 'Atencao',
      badge: 'bg-amber-50 text-amber-700 border border-amber-100 dark:bg-amber-950/30 dark:text-amber-400 dark:border-rose-900/40',
...
<template>
  <tr
    class="hover:bg-slate-50/80 dark:hover:bg-slate-900/40 cursor-pointer border-b border-slate-100 dark:border-slate-800/60 transition"
    @click="handleSelect"
  >
    <td class="px-4 py-3 min-w-[200px] max-w-[320px]">
      <div class="min-w-0">
        <p class="truncate font-bold text-slate-900 dark:text-slate-100 text-sm" :title="props.produto.descricao || ''">
        ...
        <p class="mt-0.5 text-[10px] text-slate-400 dark:text-slate-500 font-semibold">
          Empresa {{ props.produto.idempresa }} | Cod. {{ props.produto.idproduto }}/{{ props.produto.idsubproduto }}
        </p>
      </div>
    </td>
    ...
    <td class="px-4 py-3 text-right font-medium text-slate-800 dark:text-slate-200 whitespace-nowrap text-sm">
    ...
    <td class="px-4 py-3 text-right whitespace-nowrap text-sm" :class="statusInfo.textClass">
    ...
    <td class="px-4 py-3 text-right whitespace-nowrap text-sm">
      <span
        ...
        class="inline-flex items-center justify-end rounded-md border border-violet-200 bg-violet-50 px-1.5 py-0.5 text-[11px] font-black text-violet-700 dark:border-violet-500/30 dark:bg-violet-500/10 dark:text-violet-300"
    ...
    <td class="px-4 py-3 text-right text-slate-700 dark:text-slate-300 whitespace-nowrap text-sm">
    ...
    <td class="px-4 py-3 text-right font-bold text-slate-800 dark:text-slate-200 whitespace-nowrap text-sm">

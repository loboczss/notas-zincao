...
<template>
  <section class="space-y-6">
      ...
      >
        <div class="flex items-start justify-between gap-4">
          <div>
            ...
            <p class="mt-3 text-2xl font-bold tracking-tight text-slate-900 md:text-3xl">
              {{ item.valor }}
            </p>
          </div>

          <div class="rounded-2xl p-3" :class="item.badgeClass">
    ...
    <section class="grid grid-cols-1 gap-4 xl:grid-cols-[1.2fr_0.8fr]">
      <article class="rounded-[20px] border border-slate-200 bg-white p-5 shadow-sm md:p-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p class="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">
              Distribuição operacional
            </p>
            <h2 class="mt-2 text-lg font-bold tracking-tight text-slate-900 md:text-xl">
              Como as notas estão distribuídas hoje
            </h2>
          </div>

          <div class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-right">
            <p class="text-xs uppercase tracking-[0.16em] text-slate-500">Base</p>
            <p class="text-lg font-semibold text-slate-900">{{ panorama.totalNotas }} notas</p>
          </div>
        </div>
        ...
        <div class="mt-6 space-y-4">
          <div v-for="item in panorama.distribuicao" :key="item.label" class="space-y-2">
            <div class="flex items-center justify-between gap-3 text-sm">
              ...
              <span class="text-slate-500">{{ item.valor }}</span>
            </div>
      ...
      <article class="rounded-[20px] border border-slate-200 bg-white p-5 shadow-sm md:p-6">
        ...
        <div class="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-1">
          <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
            <p class="text-xs uppercase tracking-[0.16em] text-slate-500">Valor médio</p>
            <p class="mt-2 text-lg font-semibold text-slate-900">{{ formatCurrency(panorama.ticketMedio) }}</p>
          </div>
          <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
            <p class="text-xs uppercase tracking-[0.16em] text-slate-500">Concluído</p>
            <p class="mt-2 text-lg font-semibold text-slate-900">{{ panorama.percentualRetirada }}%</p>
          </div>
        </div>
      </article>
    </section>

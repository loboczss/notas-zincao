...
<template>
  <section class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
    <p class="text-[11px] font-semibold uppercase tracking-[0.28em] text-slate-500 dark:text-slate-400">Nota fiscal</p>
    <h2 class="mt-2 text-xl font-bold tracking-tight text-slate-900 dark:text-slate-100">Dados fiscais e valores</h2>

    <div class="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      <div>
        <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">Numero da nota</label>
      ...
      <div>
        <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">Serie</label>
      ...
      <div>
        <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">Data da compra</label>
      ...
      <div class="md:col-span-2 xl:col-span-3">
        <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">Chave NFe</label>
        <Input :model-value="props.chaveNfe" placeholder="44 digitos" @update:model-value="emit('update:chaveNfe', $event)" />
        <p v-if="props.errors.chave_nfe" class="mt-1 text-xs text-rose-600 dark:text-rose-400">{{ props.errors.chave_nfe }}</p>
      </div>

      <div>
        <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">Valor bruto</label>
        <Input :model-value="props.valorTotal" disabled />
      </div>

      <div>
        <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">Desconto</label>
        <Input :model-value="props.descontoTotal" placeholder="0,00" @update:model-value="emit('update:descontoTotal', $event)" />
      </div>

      <div>
        <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">Valor liquido</label>
        <Input :model-value="props.valorLiquido" disabled />
      </div>

      <div class="md:col-span-2 xl:col-span-3">
        <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">Observacoes</label>
        <textarea

...
<template>
  <section class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
    <div class="flex items-center justify-between gap-4">
      <div>
        <p class="text-[11px] font-semibold uppercase tracking-[0.28em] text-slate-500 dark:text-slate-400">Produtos</p>
        <h2 class="mt-2 text-xl font-bold tracking-tight text-slate-900 dark:text-slate-100">Itens da nota</h2>
      </div>

      <button
        type="button"
        class="inline-flex items-center gap-2 rounded-2xl border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 transition-colors hover:bg-slate-100 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800"
    ...
    <div class="mt-5 space-y-4">
      <div
        ...
        :key="`produto-${index}`"
        class="rounded-[24px] border border-slate-200 p-4 dark:border-slate-700"
      >
        <div class="mb-4 flex items-center justify-between gap-3">
          <p class="text-sm font-bold text-slate-900 dark:text-slate-100">Item {{ index + 1 }}</p>
          <button
            type="button"
            class="inline-flex items-center gap-2 rounded-xl px-3 py-2 text-xs font-semibold text-rose-600 transition-colors hover:bg-rose-50 dark:hover:bg-rose-500/10"
        ...
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          <div class="relative md:col-span-2 xl:col-span-2">
            <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">Produto</label>
            <Input :model-value="String(produto.nome || '')" @update:model-value="buscarProduto(index, $event)" />

            <p v-if="produto.id_produto_estoque" class="mt-2 text-xs font-medium text-emerald-700 dark:text-emerald-300">
          ...
          <div>
            <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">ID do produto</label>
            <Input
              :model-value="produto.id_produto_estoque ? String(produto.id_produto_estoque) : ''"
              placeholder="Vincule no estoque"
              inputmode="numeric"
          ...
          <div>
            <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">Valor unitario</label>
